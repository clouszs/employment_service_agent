...
- [阶段 8：单智能体稳固期](#阶段-8单智能体稳固期)
- [阶段 9：单智能体扩展期](#阶段-9单智能体扩展期)
- [阶段 10：多智能体触发评估](#阶段-10多智能体触发评估)
- [阶段 11：复杂业务域开发](#阶段-11复杂业务域开发)
- [模块依赖关系图](#模块依赖关系图)
- [增量优化项](#增量优化项)
...
> **完成情况**：Batch A/B 均已完成（2026-06-27），Batch C 待排期
...
- [x] `temporal_warnings` 前端可见（阶段 6/7 已完成）
- [x] `agent_chat()` 超时保护（阶段 7 已完成）
- [x] 死代码文件已删除（阶段 7 已完成）
- [x] Graph 编译仅执行 1 次（阶段 6 已完成）
- [x] 单元测试通过率 ≥ 80%（14 passed, 100%）
- [x] `AgentState` 为合法 TypedDict（操作 8-9）
- [x] 两个 Agent 端点保持独立（评估结论：暂不合并）
...
> **完成时间**：2026-06-27
> **前置条件**：阶段 8 完成
> **架构原则**：始终保持单智能体架构，所有新工具通过 `agent/tools.py` 统一注册
> **关键变更**：去债务重构，5 个工具瘦身为 3 个（见工具清单）；意图分类扩展为 9 类

### 9.1 新增 Agent 工具

| 工具 | 用途 | 实现方式 | 预估 |
|------|------|----------|------|
| `knowledge_search()` | 本地知识库检索 | 复用 `rag_service.search()` | 内置 |
| `bing_search()` | 外部搜索兜底（MCP） | 站点白名单路由 | 内置 |
| `fetch_webpage()` | 网页正文抓取（MCP） | MCP fetch 服务 | 内置 |

> **变更说明**：原 Phase 9 规划 5 个业务工具，但去债务重构后仅保留 3 个检索类工具。`generate_resume / recommend_jobs / add_calendar_event / toggle_faq_status` 均**未接线**，改为在 Phase 11 通过 REST API 独立实现（`/career` 路由 + 前端页面）。`fetch_announcement` 被 `announcement_service.get_active_config_announcements` 接管。
...
| 意图分类 | 关键词示例 | 路由目标 | 风险等级 |
|----------|------------|----------|----------|
| `chat` | 你好、谢谢 | direct_response | low |
| `unrelated` | 天气、NBA | generate_refusal | low |
| `policy_query` | 政策、落户、补贴 | search_knowledge（Bing 政务站） | high |
| `faq_query` | 怎么办理、如何申请 | search_knowledge（FAQ 优先） | medium |
| `document_query` | 文件、表格、资料 | search_knowledge（文档检索） | medium |
| `news_query` | 公告、双选会 | search_knowledge（Bing 新闻）+ fetch_announcement | medium |
| `resume_query` | 简历、CV、技能证书 | search_knowledge | medium |
| `interview_query` | 面试、面经 | search_knowledge | medium |
| `salary_query` | 薪资、待遇、五险一金 | search_knowledge（薪资类 KB） | medium |
...
- [x] 3 个 Agent 工具（knowledge_search / bing_search / fetch_webpage）注册可用
- [x] 意图分类扩展为 9 类 + 3 个兜底
- [x] AgentState 新增字段（announcements 等）不影响现有流程
- [x] 前端传历史 6 轮，跨轮矛盾检测生效
- [x] ToolCallTracker 记录在 reasoning_chain 中

---

## 阶段 10：多智能体触发评估

> **目标**：根据阶段 9 的实际运行数据，正式评估是否需要引入 Supervisor + Subgraph
> **评估日期**：2026-06-27
> **评估结论**：无需引入，继续单 Agent + REST API 层

### 评估结果：5 项检查点全部未触发

| 检查点 | 阈值 | 当前状态 | 结论 |
|--------|------|----------|------|
| 1. 独立子流程 | >= 3 个 | 单流程 12 节点 | 未触发 |
| 2. 状态隔离 | 需隔离上下文 | 扁平 AgentState 足够 | 未触发 |
| 3. 不同模型 | 不同 LLM | 全部通义千问 | 未触发 |
| 4. 工具数量 | > 8 个 | 恰好 8 个（等于阈值） | 边缘未触发 |
| 5. Prompt 长度 | > 4k tokens | 精简未超限 | 未触发 |

**结论**：5 项检查点全部未触发，维持单 Agent 架构。复杂业务域（简历/职位/日历/公告/收藏）通过 **REST API 层 + 前端页面** 独立实现，不引入多智能体。
